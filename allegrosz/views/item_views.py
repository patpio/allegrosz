...
from ..helpers.helpers import save_image_upload
...
def item(item_id):
    c = get_db().cursor()  # do pobierania nie potrzebujemy polaczenia
    c.execute('''SELECT
    ...
    if db_item:
        ...
        return render_template('item.html', item=db_item, form=delete_form)
...
def edit(item_id):
    ...
    row = c.fetchone()  # fetchone() return None if not value
    if row is None:
        abort(404)
...
def delete(item_id):
    ...
    if form.validate_on_submit():
        if row is not None:
            ...
            flash(f'Item does not exist.', 'danger')
