...
class BelongsToOtherFieldOption:
    def __init__(self, table, belongs_to, foreign_key=None, message=None):
        if not table:
            raise AttributeError('BelongsToOtherFieldOption validator need the table parameter')

        if not belongs_to:
            raise AttributeError('BelongsToOtherFieldOption validator need the belongs_to parameter')
